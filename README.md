# Hi, I'm Isabel

I'm a biomedical research professional with over ten years of experience at the Oregon National Primate Research Center (ONPRC), OHSU, where data management and analysis have been a core and growing part of my work. I recently completed an M.S. in Computational Life Sciences from Arizona State University and am relocating to New York City to transition into human health research, clinical data management, and clinical genetics.

I specialize in building reproducible data workflows, managing large-scale longitudinal research databases, and translating complex datasets into clear analytical outputs for diverse stakeholders. I am particularly interested in roles at the intersection of research coordination, clinical data management, and health informatics.

## Skills

**Languages & tools:** R (tidyverse, ggplot2, lubridate), SQL (foundational), Python (foundational), LabKey, REDCap (in progress)  
**Methods:** Longitudinal data analysis, data cleaning and QC, reproducible workflows, database management, statistical modeling, data visualization  
**Domains:** Biomedical research, behavioral health, clinical genetics, population health, public health

## Featured Projects

[**Rearing Conditions & Behavioral Health in Rhesus Macaques**](https://github.com/irbernstein/macaque-rearing-behavioral-health)  
Retrospective longitudinal study examining how early social rearing conditions predict adult behavioral health outcomes. Multi-year EHR data pipeline built in R. Manuscript in preparation.

[**Social Housing Demographics Analysis**](https://github.com/irbernstein/rhesus-social-housing-analysis)  
Demographic and social housing analysis of a captive rhesus macaque colony using R and ggplot2. Course project for BIO 579, Arizona State University.

## Currently

- Relocating to New York City
- Completing REDCap training through Vanderbilt University's clinical data management program
- Preparing manuscript on early rearing and behavioral health outcomes in rhesus macaques

---

[bernstis@ohsu.edu](mailto:bernstis@ohsu.edu) · [LinkedIn](https://www.linkedin.com/in/isabelbernstein)
